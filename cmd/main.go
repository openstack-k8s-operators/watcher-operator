...
func main() {
	...
	if os.Getenv("ENABLE_WEBHOOKS") != "false" {
		if err := webhookv1beta1.SetupWatcherAPIWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "WatcherAPI")
			os.Exit(1)
		}
		if err := webhookv1beta1.SetupWatcherWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "Watcher")
			os.Exit(1)
		}
		if err := webhookv1beta1.SetupWatcherDecisionEngineWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "WatcherDecisionEngine")
			os.Exit(1)
		}
		if err := webhookv1beta1.SetupWatcherApplierWebhookWithManager(mgr); err != nil {
