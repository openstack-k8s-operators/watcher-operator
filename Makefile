...
# openstack.org/watcher-operator-bundle:$VERSION and openstack.org/watcher-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= quay.io/$(USER)/watcher-operator
...
update-watcher-csv:
	@echo "Scaling watcher-controller-manager to 0 in CSV"
	$(eval csv=$(shell oc get csv -n openstack-operators -o name | grep watcher))
...
WATCHER_BASE_CI_IMAGE ?= quay.io/openstack-s2i-containers/openstack-watcher-base:master-latest
