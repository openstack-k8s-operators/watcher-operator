...
import (
	...
	controllers "github.com/openstack-k8s-operators/watcher-operator/internal/controller"
	webhookv1beta1 "github.com/openstack-k8s-operators/watcher-operator/internal/webhook/v1beta1"
...
var _ = BeforeSuite(func() {
	...
	err = webhookv1beta1.SetupWatcherWebhookWithManager(k8sManager)
	Expect(err).NotTo(HaveOccurred())
	err = webhookv1beta1.SetupWatcherAPIWebhookWithManager(k8sManager)
	Expect(err).NotTo(HaveOccurred())
	err = webhookv1beta1.SetupWatcherDecisionEngineWebhookWithManager(k8sManager)
	Expect(err).NotTo(HaveOccurred())
	err = webhookv1beta1.SetupWatcherApplierWebhookWithManager(k8sManager)
	Expect(err).NotTo(HaveOccurred())
