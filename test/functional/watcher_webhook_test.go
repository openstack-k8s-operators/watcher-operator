	...
	Describe("Validation of deprecated field changes", func() {
		...
		Describe("RabbitMqClusterName field changes", func() {
			It("should reject changes to RabbitMqClusterName", func() {
				...
				errs := newSpec.ValidateUpdate(*oldSpec, basePath, "test-namespace")

				Expect(errs).To(HaveLen(1))
				Expect(errs[0].Type).To(Equal(field.ErrorTypeForbidden))
				Expect(errs[0].Field).To(Equal("spec.rabbitMqClusterName"))
				Expect(errs[0].Detail).To(ContainSubstring("deprecated and cannot be changed"))
				Expect(errs[0].Detail).To(ContainSubstring("messagingBus.cluster"))
			})
		...
		Describe("NotificationsBusInstance field changes", func() {
			...
			It("should reject changes to NotificationsBusInstance", func() {
				...
				Expect(errs).To(HaveLen(1))
				Expect(errs[0].Type).To(Equal(field.ErrorTypeForbidden))
				Expect(errs[0].Field).To(Equal("spec.notificationsBusInstance"))
				Expect(errs[0].Detail).To(ContainSubstring("deprecated and cannot be changed"))
				Expect(errs[0].Detail).To(ContainSubstring("notificationsBus.cluster"))
			})
		...
		Describe("Multiple deprecated field changes", func() {
			It("should reject changes to both deprecated fields and return multiple errors", func() {
				...
				// Check for RabbitMqClusterName error
				rabbitMqErr := false
				notificationsErr := false
				for _, err := range errs {
					if err.Field == "spec.rabbitMqClusterName" {
						rabbitMqErr = true
						Expect(err.Type).To(Equal(field.ErrorTypeForbidden))
					}
					if err.Field == "spec.notificationsBusInstance" {
						notificationsErr = true
						Expect(err.Type).To(Equal(field.ErrorTypeForbidden))
					}
				}
				Expect(rabbitMqErr).To(BeTrue(), "Expected error for rabbitMqClusterName")
				Expect(notificationsErr).To(BeTrue(), "Expected error for notificationsBusInstance")
			})
		})
